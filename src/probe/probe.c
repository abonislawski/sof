...
static enum task_state probe_task(void *data)
{
	...
	if (avail > 0)
#if CONFIG_ZEPHYR_NATIVE_DRIVERS
		err = dma_reload(_probe->ext_dma.dc.dmac->z_dev, _probe->ext_dma.dc.chan->index, 0, 0, avail);
#else
...
static void probe_cb_produce(void *arg, enum notify_id type, void *data)
{
	...
	if (_probe->probe_points[i].purpose == PROBE_PURPOSE_EXTRACTION) {
		...
		if (copy_bytes > 0) {
#if CONFIG_ZEPHYR_NATIVE_DRIVERS
			ret = dma_reload(dma->dc.dmac->z_dev, dma->dc.chan->index, 0, 0, copy_bytes);
#else
...
#if CONFIG_ZEPHYR_NATIVE_DRIVERS
			if (dma_start(_probe->inject_dma[j].dc.dmac->z_dev, _probe->inject_dma[j].dc.chan->index) < 0) {
#else
