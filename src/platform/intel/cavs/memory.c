...
extern unsigned int _buffer_heap, _sof_core_s_start;
...
void platform_init_memmap(void)
{
	...
	memmap.system[0].heap = (unsigned int)&_system_heap;
	...
	memmap.system_runtime[0].heap = (unsigned int)&_system_runtime_heap;
	...
	for (i = 1; i < PLATFORM_CORE_COUNT; i++) {
		/* .system init */
		memmap.system[i].heap = (unsigned int)&_sof_core_s_start +
					((i - 1) * SOF_CORE_S_SIZE);
		...
		memmap.system_runtime[i].heap = (unsigned int)&_sof_core_s_start +
	...
	memmap.runtime[0].heap = (unsigned int)&_module_heap;
	...
	memmap.buffer[0].heap = (unsigned int)&_buffer_heap;
